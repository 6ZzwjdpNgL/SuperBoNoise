if (image_index == 1 && global.panic == false && room != rm_missing)
{
    with (obj_player1)
    {
        if (state == (97 << 0) && floor(image_index) == (image_number - 2))
        {
...
if (drop && dropstate != (128 << 0))
    return;
with (obj_player1)
{
    if (grounded && x > (other.x - 160) && x < (other.x + 160) && input_check("up") && (state == (2 << 0) || state == (105 << 0) || state == (106 << 0) || state == (123 << 0)) && (global.panic == true || global.snickchallenge == 1 || room == vir_1e))
    {
        var ex = x
        var ey = y
