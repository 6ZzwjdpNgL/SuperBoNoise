function state_player_fishswim() //state_player_fishswim
{
    fishlandtimer = 600
...
function state_player_fishdash() //state_player_fishdash
{
    fishlandtimer = 600
    goesDownSlopes = false
    move_h = input_check("right") - input_check("left")
    ...
    vsp = vdir + grav
    var bumped = 0
    if (place_meeting_collision((x + sign(hdir)), y, (1 << 0)) && (!(place_meeting((x + sign(hdir)), y, obj_destructibles))))
    {
        var _ledge = 0
    ...
    if (place_meeting_collision(x, (y + sign(vdir)), (1 << 0)) && (!(place_meeting(x, (y + sign(vdir)), obj_destructibles))))
    {
        _ledge = 0
    ...
    if place_meeting(x, y, obj_water)
    {
        ...
        if animation_end()
        {
            state = (270 << 0)
            movespeed = hsp
            target_xscale = sign(lengthdir_x(1, fishDir))
            xscale = (target_xscale != 0 ? target_xscale : xscale)
        }
    }
    else
...
function state_player_fishground() //state_player_fishground
{
    if (fishlandtimer > 0)
        fishlandtimer--
    else
...
function state_player_fishhurt() //state_player_fishhurt
{
    fishlandtimer = 600
    goesDownSlopes = false
