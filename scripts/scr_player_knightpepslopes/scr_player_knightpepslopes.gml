function state_player_knightpepslopes() //state_player_knightpepslopes
{
    if ((x + hsp) == xprevious)
        slope_buffer--
    else
    ...
    if (place_meeting_collision((x + sign(hsp)), y, (1 << 0)) && (!(place_meeting((x + sign(hsp)), y, obj_destructibles))))
    {
        movespeed = 0
        vsp = -6
